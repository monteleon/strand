...
- 2026-06-01: **writePositions UPSERTs endDate corrections on re-ingest (/code-review finding #2).** `positionIdFromParts` hashes (personId, companyId, startDate, title) — endDate is deliberately excluded so a position's identity is stable across re-ingests when the owner extends or shortens a role on LinkedIn. With the prior `onConflictDoNothing()` that stability meant a corrected endDate (e.g. "2024-01" → "2024-06") was silently dropped on re-ingest because the second insert hit the PK collision and was discarded; the user would never see their correction land. Switched to UPSERT on the mutable post-identity fields (endDate + current — `current` is derived from endDate at parse time per finding #5's fix). startDate/title/companyId changes still produce a new row with a new id; that's correct because those changes amount to "a different position." Re-ingesting the most recent export is now a one-shot repair for any user previously affected by this bug. 4 new tests in src/lib/linkedin/ingest.test.ts under `writePositions — endDate corrections land via UPSERT`: first ingest writes 1 row with original endDate; corrected endDate re-ingest stays at 1 row with updated endDate (the bug case); cleared endDate re-ingest updates endDate=null AND current=true together; idempotent re-ingest stays at 1 row with same values. writePositions exported for testability. Full suite 128/128.
- 2026-06-01: **graph-filters honours `/graph?company=` (explicit empty) as a clear, not a fall-back (/code-review finding #9).** Pre-fix line 145 used `searchParams.get("company")?.trim() || null ?? initialCompanyId` — JavaScript precedence runs `||` first, so the empty string short-circuited to null and then `??` fell back to the SSR-seeded `initialCompanyId`. Result: a URL like `/graph?company=&kinds=manual` (from an external link or a hand-edited bookmark) silently re-applied the previous company filter instead of clearing it; the active-company chip would re-appear with no obvious way to make it stick at "none." New helper `parseCompanyParam(raw, initialCompanyId)` distinguishes three URL shapes explicitly: param absent (`raw === null`) → `initialCompanyId` fall-back; param present and (after trim) empty → `null` (clear); param present with value → trimmed value. Same `parseFooParam` shape as the other inline parsers in this file. Exported for testability. 7 unit tests in new `src/components/graph-filters.test.ts` cover the bug case (`?company=` → null), whitespace-only, absent + initial, absent + no initial, plain value, whitespace-padded value, and present-value-overrides-initial. Full suite 124/124.
- 2026-06-01: **personInspector ORDER BY chain now ends with `po.id ASC` final tiebreaker (/code-review finding #6).** Pre-fix the inspector's current-employer query stopped at `COALESCE(po.start_date, '0000-00-00') DESC`, so on identical (origin, current, start_date) SQLite returned a nondeterministic row that could disagree with the same chain in `graph.ts:222` and `graph.ts:374` (both of which DO include `po.id ASC`). User-visible symptom: the right-rail inspector and the /graph node cluster colour for the same person could show different current employers across re-renders. Fix: append `po.id ASC` to the ORDER BY so the lex-smallest position id wins deterministically — same canonical chain as everywhere else. New test file `src/lib/queries/personInspector.test.ts` with 5 tests: the tiebreaker scenario (two declared current positions, same start_date, different ids → lex-smallest id's company wins) plus 4 regression-safety cases (solo position; current beats past; declared beats synthesised; later start beats earlier). All subprocess-isolated against a temp DB so they don't pollute `data/strand.db`. Full suite 117/117.
- 2026-06-01: **parsePositions `current` now requires a startRaw, not just an absent endRaw (/code-review finding #5).** Pre-fix `current: !endRaw` flipped to true for any Positions.csv row where Finished On was blank — including rows with Title + Description but NO Started On either (Volunteer entries are a common shape). Those rows then persisted with current=true and joined the `shared_employer_currently` pool in deriveSharedEmployerEdges, inflating confidence on connection-pairs that happened to share the same company (0.5 → 0.7 when paired with one of the owner's declared current positions). New rule: `current: Boolean(startRaw) && !endRaw` — "current since when?" is unanswerable without a start date. Matt's actual DB happens to have 2 such undated declared positions (Head of IT Development, Supervisor Project Management) but they were already current=0 (his CSV must carry non-empty Finished On strings for them), so no backfill is needed for his data; the fix is forward-looking for any future ingest. 5 new unit tests in parse.test.ts cover the 4 corner cases (start+end → past, start no-end → current, NO dates → false [the bug], no-start has-end → false [unchanged]) plus a whitespace-only Started On guard. parsePositions exported for testability. Full suite 112/112.
- 2026-06-01: **One-shot repair script for tenants already orphaned by the pre-v0.4.2 writeOwner bug.** `scripts/repair-owner-orphans.ts` detects any people row id that appears in `connections.from_person_id` but is NOT the current `tenants.ownerPersonId` (connections.from_person_id is invariantly the owner per ISC-38) and rewrites every FK reference onto the current owner: connections via INSERT-OR-IGNORE-then-DELETE (handles unique-key collisions cleanly), positions via UPDATE person_id, manual_edges via re-canonicalised INSERT-OR-IGNORE-then-DELETE (preserves the `person_a < person_b` lex invariant + skips self-loops if the stranded owner had an asserted edge with the current owner), messages via UPDATE from_person_id / to_person_id. derived_edges referencing the stranded id are dropped and `deriveSharedEmployerEdges` runs once per repaired tenant to rebuild. The stranded people row is deleted last so the onDelete cascades have nothing left to fire on. Default mode is DRY-RUN — `--apply` mutates. Dry-run on Matt's actual DB confirms zero stranded owners (he has never been bitten). 8-test integration suite at `scripts/repair-owner-orphans.test.ts` synthesises an orphan state (2 owner rows, tenant points at NEW, every FK references OLD), runs both passes via Bun.spawn against an isolated temp DB, and asserts the rewrite is complete + the stranded row is gone. Full suite 107/107.
- 2026-06-01: **writeOwner is now idempotent across rename re-ingests.** Surfaced by today's xhigh /code-review as the single most catastrophic finding. The old derivation hashed the owner's display name into their primary key (`ownerPersonId(tenantId, fullName) = sha1(tenantId|owner|fullName)`), so a re-ingest after the owner renamed themselves on LinkedIn (marriage, accent fix, formal↔casual) inserted a second owner row with a different id, flipped `tenants.ownerPersonId` to the new id, and left every existing connection/position/manual_edge FK reference pointing at the now-dead old id — every read query then returned owner-empty results until manual DB surgery. New behaviour in writeOwner: look up the existing tenant's `ownerPersonId`; if that people row still exists, UPDATE its fullName/headline/lastSeen in place and preserve the id. Only the very first ingest (or recovery from a corrupted state where the referenced row vanished) takes the original sha1-from-name insert path. Two downstream call sites that also recomputed `ownerPersonId(tenantId, fullName)` — `backfillOwnerLinkedinUrl` and `writeMessages` — now read the owner id from `tenants` instead, single source of truth. 5 new tests under `writeOwner — rename re-ingest preserves owner id (v0.4.2)` cover: id unchanged across rename, no second owner row created, fullName + headline updated to the new values, pre-existing connection FK still resolves to the same owner. Full suite 99/99 green.
- 2026-06-01: **Company-filter edge floor auto-relaxes to 0 when the owner is NOT a declared employee of the filtered company.** Follow-up to the candidate-set fix earlier today. With colleagues now in the node set for KPMG/Lloyds, the rail showed `edges: 0` because their `shared_employer_currently` edges sit at 0.5 confidence — below the slider's 0.7 default. New rule: when `companyId` is set AND `ownerId ∉ allowedPersonIds`, `effectiveMinConfidence = 0` for both the candidate query and the derived-edges fetch. When the owner IS at the company (PwC España), the slider value is honoured because relaxing would push the rendered edge count from 143 to ~10,300 — the exact hairball the W6 EDGE_CAP comment warns against. Meta reports the *effective* floor (so the rail stat tells the truth about what was applied) — divergence between the slider value and the rail "confidence floor 0.00" is the user-visible signal that the company filter unlocked within-company edges. Curl regression: KPMG España 0→10 edges, Lloyds Bank Intl 0→45 edges, PwC España 146→146 edges (no hairball), `/graph` default 153→153 edges (no change). 10/10 graph unit tests still pass.
- 2026-06-01: **Company filter no longer requires intersection with the edge-qualified candidate set.** Before: candidate set = `(manual_edges ∪ derived_edges ≥ 0.7) ∩ allowedPersonIds`. For companies where the owner has a declared position (PwC España), every colleague picks up a 0.7+ derived edge to the owner and the intersection is the full colleague set. For companies where the owner is NOT a declared employee (KPMG, Lloyds Bank International), the only derived edges among colleagues are `shared_employer_currently` at 0.5 confidence — below the 0.7 candidate floor — so the intersection was empty and the graph rendered just the owner. After: when a `companyId` filter is active, the candidate set IS the `allowedPersonIds` set; the edge-qualified candidate metrics still drive ORDER within the cap (most-connected first) but no longer gate the SET. ISC-373 ("owner always in, even disconnected") preserved. ISC-372 ("filter then cap") preserved. ISC-387/388/389/390 unit tests still pass. Curl regression: KPMG España 1→6, Lloyds Bank International 1→10, PwC España 144→144. Follow-up open question: edges among colleagues at owner-absent companies still hide behind the 0.7 default `minConfidence` slider — should the slider auto-relax to 0 when a `companyId` filter is set so the within-company `shared_employer_currently` (0.5) edges render?
- 2026-06-01: **`/graph` filter URL writer switched from `history.pushState + router.refresh` to `router.push(url, { scroll: false })`.** Symptom: selecting a company in the filter panel did not refresh the graph until the user manually re-submitted the URL via the address bar. Root cause: `router.refresh()` re-fetches the RSC payload against the **router's tree URL**, not `window.location` — `history.pushState` updates the browser URL bar but never the tree URL, so refresh re-fetched the OLD URL and the company filter silently no-op'd. `router.push` is the canonical App Router navigation method, updates tree URL + triggers RSC re-fetch in one call, and the page already declares `dynamic = "force-dynamic"` so there's no client cache to dedup against. `scroll: false` preserves graph viewport across slider/chip edits. The older "Next dedupes query-only push/replace" comment was a pre-14.2 footgun the codebase no longer needs to route around. Same `apply()` function is shared by every filter control (kinds chips, sliders, cap dropdown, company picker, scope toggle, clear button), so this fixes the silent-no-op for all of them, not just company.
